...
def randomize_cells(world, hl, inf, imm):
    """
    sets a random position for all healthy and infected cells given (i.e., the array is randomly filled with os, 1s, 2s and 3s)
    """
...
def infect(world, coord, tuples, frames):
    ...
    for p in poss:
        ...
        if x in healthy:
            chance = np.random.choice([1, 2], size=None, replace=True, p=[0.9, 0.1]) # set the probability of infection: 1 = healthy, 2 = infected; p[no infection, infection]
            world[x] = chance
            if chance == 2:
...
def recover(world, tuples, frames):
    ...
    If the difference is greater or equal than some specific value, then the cell change its state to either inmune
    or dead
    """
    for t in tuples:
        if frames - t[1] >= 20:
...
# Preprocessing: every operations is computed in order to run the simulation smoothly and avoid stuttering
healthy = 200
infected = 2
immune = 1
my_world = world_dimensions(30, 30)  # creates a world with m x n dimensions
my_world = randomize_cells(my_world, healthy, infected, 0)  # number of healthy, infected, and immune cells randomly distributed in the world
tuples = list_of_tuples(my_world)
...
# add time at the top left corner of the figure
...
# Statistics
p = population
t = len(list(zip(*(np.where((my_world > 1) & (my_world <= 4)))))) - 2
d = len(dead_particles(my_world))
prob_t = t / p * 100
prob_d = d / t * 100
